    ...
    if "Task" not in df_p.columns:
        ...
        if not df_p.empty:
            ...
            df_p["Display"] = df_p.apply(
                lambda x: "\u00A0" * 6 * int(x["Level"]) + str(x["Task"]),
                axis=1
            )
...
orden_y = [str(i) for i in df_p["row_pos"].tolist()]
        ...
        cols_to_drop = [c for c in ["L0_idx", "L1_idx", "row_pos", "row_pos_str", "Display"] if c in df_t_show.columns]
        ...
        with c2:
            with st.expander("🗑️ Eliminar Tarea"):
                ...
                if st.button("Confirmar Borrado"):
                    ...
                    cols_to_drop_f = [c for c in ["L0_idx", "L1_idx", "row_pos", "row_pos_str", "Display"] if c in df_f.columns]
