...
# Forzar a que las tablas ocupen todo el ancho disponible
    ...
    if data_t:
        ...
        if not df_chart.empty:
            chart = alt.Chart(df_chart).mark_bar(cornerRadius=3).encode(
                x=alt.X('Start:T', title="Inicio"), x2='End:T',
...
if ws_red:
    ...
    df_r_ed = st.data_editor(df_r, hide_index=True, use_container_width=True, num_rows="dynamic",
                             column_config={"USO": st.column_config.TextColumn("Uso", width="large"), "ESTADO": st.column_config.CheckboxColumn("OK")})
...
if ws_hitos:
    ...
    with t1:
        ...
        ed_off = st.data_editor(df_off, hide_index=True, use_container_width=True, num_rows="dynamic", key="ed_off", column_config=conf_h)
    with t2:
        df_on = df_h[df_h["TIPO"] == "Onshore"].copy()
        ed_on = st.data_editor(df_on, hide_index=True, use_container_width=True, num_rows="dynamic", key="ed_on", column_config=conf_h)
...
if ws_spare:
    ...
    df_s_ed = st.data_editor(df_s, hide_index=True, use_container_width=True, num_rows="dynamic", column_config=conf_s, key="ed_spare_final")

    if st.button("💾 Guardar Inventario"):
        df_final_s = df_s_ed.dropna(subset=["DESCRIPCION"])
